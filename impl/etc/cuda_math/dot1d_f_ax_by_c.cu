...
static __global__ void kerd_f_ax_by_c__shared_16__t(
	uint T,			//KERD(T)
	//
...
{
	ASSERT(T%BLOQUE==0);
	ASSERT(X0%BLOQUE==0 && X1%BLOQUE==0);
	ASSERT(Y%BLOQUE==0);
	//
	kerd_f_ax_by_c__shared_16__t<<<dim3(KERD(Y, BLOQUE), KERD(T, BLOQUE)), dim3(BLOQUE, BLOQUE)>>>(
		T,
		//
...
static __global__ void d_kerd_f_ax_by_c__shared_16___dX( //pour a b c
	uint T,
	//
...
static __global__ void d_kerd_f_ax_by_c__shared_16___dA(
	uint T,
	//
	float * x, uint X_vars, uint X, uint depart_x, float * dx, uint x_depart__t,
	//
...
static __global__ void d_kerd_f_ax_by_c__shared_16___db(
	uint T,
	//
	float *  y, uint Y__vars, uint  Y, uint depart__y, float * dy, uint y__depart__t,
	float *  l, uint L__vars,          uint depart__l,
	...
	uint activation)
{
	...
	d_kerd_f_ax_by_c__shared_16___dX<<<dim3(KERD(X0, BLOQUE), KERD(T, BLOQUE)), dim3(BLOQUE, BLOQUE)>>>(
		T,
		//
	...
	d_kerd_f_ax_by_c__shared_16___dX<<<dim3(KERD(X1, BLOQUE), KERD(T, BLOQUE)), dim3(BLOQUE, BLOQUE)>>>(
		T,
		//
	...
	d_kerd_f_ax_by_c__shared_16___dA<<<dim3(KERD(X0, BLOQUE_MAX), KERD(Y, BLOQUE_MAX), DIV(T,BLOQUE_MAX)), dim3(BLOQUE_MAX, BLOQUE_MAX,1)>>>(
		T,
		//
		x0, X0_vars, X0, depart_x0, dx0, x0_depart__t,
		//
		...
		activation);
	//
	d_kerd_f_ax_by_c__shared_16___dA<<<dim3(KERD(X1, BLOQUE_MAX), KERD(Y, BLOQUE_MAX), DIV(T,BLOQUE_MAX)), dim3(BLOQUE_MAX, BLOQUE_MAX,1)>>>(
		T,
		//
		x1, X1_vars, X1, depart_x1, dx1, x1_depart__t,
		//
	...
	d_kerd_f_ax_by_c__shared_16___db<<<dim3(KERD(Y, BLOQUE_MAX), KERD(T,BLOQUE_MAX)), dim3(BLOQUE_MAX,BLOQUE_MAX)>>>(
		T,
		//
		y, Y__vars, Y, depart__y, dy, y__depart__t,
		l, L__vars,    depart__l,
